...
protected $compress;           // compression flag
    protected $iconv = true;       // force UTF-8 handling
...
function __construct($orientation='P', $unit='mm', $size='A4')
{
	...
	$this->ws = 0;
	$this->iconv = true; // Force UTF-8 support
	// Font path
	...
	// Get width of a string in the current font
        ...
        for($i=0;$i<$l;$i++) {
            ...
            if(isset($cw[$char])) {
                ...
                $w += $cw['?'] ?? 500; // Default width for unknown chars
            }
        }
...
function AddFont($family, $style='', $file='', $dir='')
{
	...
	if(!empty($info['file']))
	{
		...
		if($info['type']=='TrueType')
            ...
            $length1 = $info['size1'] ?? $info['originalsize'] ?? 0;
            $length2 = $info['size2'] ?? 0;
            $this->FontFiles[$info['file']] = array('length1'=>$length1, 'length2'=>$length2);
	}
...
protected function _isascii($s)
{
	...
	for($i=0;$i<$nb;$i++)
	{
        // Allow extended characters (Cyrillic, etc.)
        if(ord($s[$i])>127 && ord($s[$i])<192)
            return false;
	}
	return true;
}
	...
	protected function _putfonts()
	{
		foreach($this->FontFiles as $file=>$info)
		{
			...
			if(file_exists($file)) {
				...
				// Skip missing AFM files for core fonts
				$font = '';
			}
